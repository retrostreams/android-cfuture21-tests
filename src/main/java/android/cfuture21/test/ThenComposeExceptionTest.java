...
public class ThenComposeExceptionTest {
    ...
    static final BiFunction<CompletableFuture<String>, CompletableFuture<String>, CompletableFuture<String>> THEN_COMPOSE = (
            f, fe) -> f.thenCompose(s -> fe);

    static final BiFunction<CompletableFuture<String>, CompletableFuture<String>, CompletableFuture<String>> THEN_COMPOSE_ASYNC = (
            f, fe) -> f.thenComposeAsync(s -> fe);

    static final Consumer<CompletableFuture<String>> COMPLETE_EXCEPTIONALLY = f -> f
            .completeExceptionally(new RuntimeException());

    static final Consumer<CompletableFuture<String>> NOP = f -> {
    };
    ...
    static Object[][] actionsDataProvider() {
        ...
        data.add(new Object[] { "thenComposeAsync and completeExceptionally", NOP, THEN_COMPOSE_ASYNC,
                COMPLETE_EXCEPTIONALLY });
        data.add(new Object[] { "completeExceptionally and thenCompose", COMPLETE_EXCEPTIONALLY, THEN_COMPOSE, NOP });
        data.add(new Object[] { "completeExceptionally and thenComposeAsync", COMPLETE_EXCEPTIONALLY,
                THEN_COMPOSE_ASYNC, NOP });

        return actionsDataProvider = data.toArray(new Object[0][]);
    }

    @SuppressWarnings("unchecked")
    public void testThenComposeException() throws Exception {
        Object[][] actions = actionsDataProvider();
        for (Object[] action : actions) {
            testThenCompose((String) action[0], (Consumer<CompletableFuture<String>>) action[1],
                    (BiFunction<CompletableFuture<String>, CompletableFuture<String>, CompletableFuture<String>>) action[2],
                    (Consumer<CompletableFuture<String>>) action[3]);
        }
    }

    private void testThenCompose(String description, Consumer<CompletableFuture<String>> beforeAction,
        ...
        CompletableFuture<String> f_whenComplete = f_thenCompose.whenComplete((r, e) -> eOnWhenComplete.set(e));
        ...
        System.out.println(ThenComposeExceptionTest.class.getName() + " " + description + " SUCCESS");
    }
}
